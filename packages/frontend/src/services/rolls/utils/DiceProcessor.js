...
class DiceProcessor {
  ...
  static applyFavoredOrIllFavored(diceResults, isFavored) {
    // Get all d12 results
    const d12Results = diceResults.filter(result => result.dieSize === DIE_TYPE.D12)
  ...
  static markMaxValueDice(diceResults) {
    diceResults.forEach(result => {
      result.rolledMaxValue = result.dieSize === result.dieRollValue && !result.isDropped
    })
    return diceResults
  }
  ...
  static calculateTotal(diceResults, twiceWearyFilter = false) {
    return diceResults.reduce((sum, result) => {
      ...
      if (result.dieSize === DIE_TYPE.D12 && result.dieRollValue === SPECIAL_ROLLS.MORTE) {
        return sum
      }
      
      // Apply Twice Weary filter if enabled (d6 rolls of 1-3 don't count)
      if (twiceWearyFilter && result.dieSize === DIE_TYPE.D6 && result.dieRollValue <= TWICE_WEARY_THRESHOLD) {
        return sum
      }
  ...
  static checkAutoFail(diceResults, isTwiceMiserable, isFavored) {
    ...
    const d12Rolls = diceResults
      .filter(result => result.dieSize === DIE_TYPE.D12)
