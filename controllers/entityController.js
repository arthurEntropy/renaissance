const { getDirectory, getAllDataByDirectory, saveFile, deleteFile } = require('../utils/fileService.js');
...
const updateEntity = (entity) => (req, res) => {
  try {
    ...
    if (existingEntity) {
      ...
      return res.status(200).send(`${entity.slice(0, -1)} updated successfully.`);
    } else {
...
const deleteEntity = (entity) => (req, res) => {
  try {
    ...
    if (entityToDelete) {
      ...
      return res.status(200).send(`${entity.slice(0, -1)} deleted successfully.`);
    } else {
