    ...
    <div class="engagement-dice-content">
      <div v-if="characterEngagementDice.length > 0" class="dice-display">
        <span
          ...
          class="dice-icon"
          :title="
            diceSourceInfo[index]
              ? `${diceSourceInfo[index].name}: d${diceSourceInfo[index].die}`
              : `d${die}`
          "
        >
      ...
      <div v-else class="no-dice-message">No engagement dice available</div>
    </div>

    <!-- Engagement Successes Section -->
    <div
      v-if="uniqueEngagementSuccesses.length > 0"
      class="engagement-successes-section"
    >
  ...
  computed: {
    equipmentWithDice() {
      ...
      this.character.equipment.forEach((characterEquip) => {
        ...
        const fullEquipment = this.allEquipment.find(
          (eq) => eq.id === characterEquip.id,
        )

        // Check if the equipment has engagement dice and is active
        const isActive =
          characterEquip.isCarried !== false &&
          characterEquip.isWielding !== false
        if (
          isActive &&
          fullEquipment &&
          fullEquipment.engagementDice &&
          fullEquipment.engagementDice.length > 0
        ) {
    ...
    allEquipmentEngagementSuccesses() {
      ...
      this.character.equipment.forEach((characterEquip) => {
        ...
        if (
          isActive &&
          fullEquipment &&
          fullEquipment.engagementSuccesses &&
          fullEquipment.engagementSuccesses.length > 0
        ) {
    ...
    uniqueEngagementSuccesses() {
      ...
      return uniqueIds
        .map((id) =>
          this.allEngagementSuccesses.find((success) => success.id === id),
        )
