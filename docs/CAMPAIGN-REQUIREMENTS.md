...
- GMs see all concepts on concept pages regardless of campaign mode (they authored the filter and should never be blocked by it).

### 4.4 Bestiary in campaign mode

- The bestiary always shows all beast templates (they are world reference material).
- Beast instances generated by the GM for this campaign are not visible to players unless explicitly shared **[DECIDE]**.
...
- World Elements are available for curation but their inclusion does not currently filter any UI behavior (they are lore reference only). **[DECIDE]** Future: entering a world element's location into campaign mode could filter what Places appear on the cultures page.
...
- GMs cannot edit player characters **[DECIDE: or can they? edge case for co-GM corrections]**.
